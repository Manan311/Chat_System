...
bool buffer_full[MAX_SOCKETS] = { false }; 
bool bf = false;
...
void winsock_server::rec_mode(int socket_number) {
	while (true) {
		...
		if (strcmp(message_buffer, "quit") == 0) {
			...
			active_sockets[socket_number] = 0;
			buffer_full[socket_number] = true;
			break;
		}
		for (bool &ck : buffer_full) {
			ck = true;
		}
	}
}

void winsock_server::send_mod(int socket_number) {
	while (true) {
		if (buffer_full[socket_number]) {
			if (strcmp(message_buffer, "quit") == 0) {
				closesocket(this->connection_socket[socket_number]);
				active_sockets[socket_number] = 0;
				buffer_full[socket_number] = false;
				for (bool i : buffer_full) {
					bf = bf || i;
				}
				if (!bf) {
...
//gets and prints any received messages
void winsock_client::get_messages() {
	while (true) {
		memset(rx_buffer, 0, 128);
		char rx_message[128] = "";
		strcpy(rx_message, this->receive_message());
		Print(rx_message);
	}
}
